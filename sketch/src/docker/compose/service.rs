...
impl ServiceData {
  pub fn as_config(self) -> Result<Service, &'static str> {
    match self {
      ServiceData::Id(_) => Err("Cannot convert preset ID to preset data"),
      ServiceData::Config(docker_service_preset) => Ok(docker_service_preset.config),
    }
  }
}
...
impl DockerServicePreset {
  pub fn process_data(
    ...
    let merged_preset = merge_presets(Preset::DockerService, id, self, store, &mut processed_ids)?;
