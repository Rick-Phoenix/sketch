...
impl Config {
	pub async fn create_ts_monorepo(
		...
		let (package_json_id, package_json_preset) =
			match root_package.package_json.unwrap_or_default() {
		...
		if let Some(oxlint_config) = root_package.oxlint
			&& !matches!(oxlint_config, OxlintConfigSetting::Bool(false))
		{
		...
		if !root_package.with_templates.is_empty() {
